    ...
    (function(){
        ...
        $("#ctf_form").submit(function(){
            ...
            if($("#ctf_content").val() == cont_v) {
                $("#ctf_tips").html('<div class="alert alert-red margin-top"><strong>操作失败：</strong>请填写评论内容！</div>');
                return false;
            }
            if(!author_v) {
                $("#ctf_tips").html('<div class="alert alert-red margin-top"><strong>操作失败：</strong>请填写昵称！</div>');
                return false;
            }
            var _this = $(this);
            $.post(_this.attr("action"), _this.serialize(), function(data){
                try{
                    ...
                    var json = eval("("+data+")");
                    if(json.kong_status) {
                        $("#ctf_tips").html('<div class="alert alert-green margin-top"><strong>恭喜：</strong>'+json.message+'</div>');
                        setTimeout(function(){
                        ...
                        if(json.kp_error) {
                            ...
                            $("#ctf_tips").html('<div class="alert alert-red margin-top"><strong>操作失败：</strong>'+json.message+'</div>');
                        }
                    }
                }catch(e){
                    alert(data);
